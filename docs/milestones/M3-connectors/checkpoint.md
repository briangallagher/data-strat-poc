...
| PG-040 | Single shared Milvus instance for all collections | POC uses one Milvus deployment with separate collections (like tables). Different RAG apps may need physical isolation (separate Milvus instances per app/team/tenant). | Support per-app Milvus deployment with registry mapping collection → Milvus endpoint. Options: shared instance (current), dedicated instance per app, or partition-key isolation. | Add optional `milvus_endpoint` to collections table; orchestrator routes to correct instance. Decision depends on multi-tenancy, SLA, and security requirements. | Open |
| PG-041 | Marquez job naming: unified graph vs per-job run history | Current: collection-specific job names (acquire_documents/underwriting_guidelines) give a unified graph where shared docs bridge collections. Trade-off: lose the "compare runs of the same job" view. Alternative: generic job names (acquire_documents) with collection as a facet — gives run history but disconnected graphs. | Evaluate both approaches in production; may need Marquez UI customisation or a separate visualisation layer to support both views. | Decision depends on whether ops teams need run-over-run comparison (generic names) or end-to-end provenance visibility (collection-specific names). Both are valid. | Open |
