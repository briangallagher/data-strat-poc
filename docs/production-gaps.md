...
| PG-015 | No RBAC automation for pipeline SA | Pipeline SA needs RayJob, KServe, HardwareProfile permissions; created manually via `oc` | RBAC manifests shipped with deployment and automated | **Fixed:** `manifests/rbac/pipeline-rbac.yaml` provides Role/RoleBinding for `pipeline-runner-dspa`. | **Closed** |
...
| PG-017 | 500Gi MinIO PVC from Milvus Helm defaults | Helm default for Milvus internal MinIO — way oversized | Right-sized storage (10-50Gi for POC) | **Fixed:** `manifests/milvus/values.yaml` sets `minio.persistence.size: 10Gi`. | **Closed** |
...
|------|-------|------------------|------|
| KFP / Pipelines Platform | 2 | 2 (PG-014 mitigated, PG-015 closed) | 0 |
| Ray / RayData / Docling | 2 | 0 | 2 |
| Milvus / Vector Storage | 5 | 1 (PG-017 closed) | 4 |
