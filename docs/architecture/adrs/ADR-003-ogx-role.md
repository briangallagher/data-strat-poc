...
1. **Ingest path** — OGX Vector I/O API accepts raw text, handles embedding and Milvus writes in a single API call. Used in early iterations.
...
The initial approach used OGX for both. The Ray team's PR #53 (merged, 4K+ lines, reviewed by 3 engineers) bypasses OGX entirely for ingest — it writes directly to Milvus using local `sentence-transformers` or a vLLM `/v1/embeddings` endpoint. OGX is not even a dependency in their pipeline.

The question: should we use OGX for the ingest path, the query path, both, or neither?
...
- Early iterations proved it works
...
- **Not what the Ray team ships**: the merged pipeline components don't use OGX; adopting their pattern means diverging from the initial approach but aligning with the upstream community
...
### Direct Milvus Writes (the Ray team's Pattern) — Pros
...
**Use direct Milvus writes for the ingest path (the Ray team's pattern). Reserve OGX for the query path (M4).**

Specifically:
- **M1 (Ingest):** Adopt the Ray team's `parse_and_chunk` → S3 → `ingest_to_milvus` pattern. Embedding via vLLM endpoint (KServe InferenceService) or local sentence-transformers as fallback. Direct pymilvus writes to Milvus.
...
- **OGX Vector I/O is not used.** If the OGX team later GA the Vector I/O API with batch control and error handling, re-evaluate.

## Alternatives Considered

| Option | Pros | Cons | Why Not |
|--------|------|------|---------|
| **A: OGX for both (initial pattern)** | Simpler pipeline; proven in early iterations | Opaque, coupled, Dev Preview, diverges from the Ray team's merged code | Control and debuggability matter for production-grade |
...
| **C: OGX for embedding only, direct Milvus writes** | OGX handles embedding model; we handle writes | Still coupled to OGX for ingest; not what the Ray team ships | Half-measure; doesn't solve the core coupling issue |
...
| **Chain 1 (Answer Provenance)** | `answer → chunk → pipeline_run_id → KFP run → source doc`. Fully traceable because every link is under our control. | Same chain, but the `pipeline_run_id` on the vector depends on OGX preserving it. Untested with custom metadata — the initial OGX approach had `pipeline_run_id` always null in query traces. |
...
The two-layer lineage architecture (DEC-009) separates pipeline-time lineage (Marquez/OpenLineage) from query-time lineage (MLflow traces). The bridge between them is `pipeline_run_id` on every Milvus vector.
...
- Aligns with the Ray team's components — potential to contribute improvements back to `pipelines-components`.
...
- The Scenario B spec assumes OGX for both ingest and query. This project deviates on ingest for production-grade and lineage reasons — document this clearly when presenting to stakeholders.
...
| Source | Link |
|--------|------|
| Prior OGX usage (serial ingest) | `data-strategy-poc/scripts/ingest-corpus.py` |
| Prior OGX usage (Ray ingest) | `data-strategy-poc/scripts/ray-ingest.py` |
| the Ray team's ingest components (no OGX) | [pipelines-components #53](https://github.com/opendatahub-io/pipelines-components/pull/53) |
| Prior DEC-002 (OGX in MVP) | `data-strategy-poc/docs/decisions.md` |
